/**
 ...
 @License：MIT
  ...
  change： 861   update方法中 刷新单行toolbar 
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,url: options.url
        ,data: JSON.stringify( $.extend(options.where, params) )
        ,headers: {
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){
              ...
              if(item3.checkbox){
                return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" '+ function(){
                  ...
                  // if(item3[checkName]){
                  //   item1["check"] = item3[checkName];
                  //   return item3[checkName] ? 'checked' : ''; 
                  // }
                  return item1["check"] ? 'checked' : '';                   
                }() +'>';
              }
              // if(item3.checkbox){
              //   return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" '+ function(){
          ...
          layui.each(fields, function(key, value){
            if(key in data){
              ...
              that.eachCols(function(i, item2){
                /*新增部分*/
                if(item2.toolbar == "#barDemo") {
                  tr.find("td[data-field='" +i+ "']").find(".layui-table-cell").html(laytpl($(item2.toolbar).html()||'').render(data));
                }
