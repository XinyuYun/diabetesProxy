  ...
  Tree.prototype.tree = function(elem, children) {
    ...
    layui.each(nodes, function(index, item) {
      ...
      var li = $(
        [
          "<li " +
            (item.spread ? 'data-spread="' + item.spread + '" ' : " ") +
            (item.check ? 'data-check="' + item.check + '" ' : " ") +
            ">",
          //展开箭头
          (function() {
            return hasChild
            ...
            return options.check
              ? '<i class="layui-icon layui-tree-check">' +
                (options.check === "checkbox"
                  ? item.check ? icon.checkbox[1] : icon.checkbox[0]
  ...
  Tree.prototype.spread = function(elem, item) {
    ...
    var check_child = function(item, isSelect, dom) {
      if (item.child && item.child.length > 0) {
        ...
        item.child.forEach(function(i, index) {
    ...
    if (options.check) check_arr.on("click", check);
      ...
      $(document)
        .on("mousemove", function(e) {
          ...
          if (move.from) {
            ...
            dragElem
              .addClass("layui-show")
              .html(move.from.elem.children("a").html());
            dragElem.css({
              left: e.pageX + 10,
              top: e.pageY + 10
            });
